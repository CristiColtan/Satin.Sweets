<template>
    <div class="animate-fade-in-down rounded-lg bg-white p-4 shadow">
        ...
        >
            <div class="flex items-center">
                ...
                <span class="ml-3 font-sans sm:text-lg"
                    >Found
                    <span class="text-rosegold-900">{{ plushies?.total }}</span>
                    items
                </span>
            </div>
        ...
        <table class="w-full table-auto">
            <thead>
                <tr>
                    <TableHeaderCell
                        ...
                        field="id"
                        @click="sortPlushies('id')"
                        >ID</TableHeaderCell
                    ><TableHeaderCell
                        :sort-direction="sortDirection"
                        :sort-field="sortField"
                        field="name"
                        @click="sortPlushies('name')"
            ...
            <tbody v-if="plushies.loading || !plushies.data.length">
                <tr>
                    <td colspan="5">
                        <Spinner
                        ...
                        >
                            Nu exista plușuri înregistrate!
                        </p>
                    </td>
                </tr>
            </tbody>
            <tbody v-else>
                <tr v-for="(plushie, index) of plushies.data" class="text-lg">
                    <td class="border-b border-gray-300 p-2">
                        {{ plushie.id }}
                    </td>
                    <td
                        class="border-b border-gray-300 p-2"
                        v-html="plushie._formatted?.name || plushie.name"
                    ></td>
                    <td class="border-b border-gray-300 p-2">
                        <img
                            v-if="
                                (Array.isArray(plushie.media) &&
                                    plushie.media.length > 0 &&
                                    plushie.media[0].full_url) ||
                                (Array.isArray(plushie.images) &&
                                    plushie.images.length > 0 &&
                                    plushie.images[0].url) ||
                                plushie.image
                            "
                            :alt="
                                plushie.media?.[0]?.alt_text ||
                                plushie.images?.[0]?.alt_text ||
                                plushie.image ||
                                plushie.title
                            "
                            :src="
                    ...
                    <td class="border-b border-gray-300 p-2">
                        <div
                            :style="{ backgroundColor: plushie.hex_code }"
                            class="h-10 w-10"
                        ></div>
                    </td>
                    <td class="border-b border-gray-300 p-2">
                    ...
                    <td class="border-b border-gray-300 p-2">
                        <div class="flex flex-wrap gap-2">
                            <span
                                v-for="cat in plushie.categories"
                                :key="cat.id ?? cat"
        ...
        >
            <div
                v-if="plushies.data.length"
                class="flex items-center font-sans text-[14px] whitespace-pre-wrap"
            >
                Showing from
                <span class="text-rosegold-500"> {{ plushies.from }} </span> to
                <span class="text-rosegold-500">{{ plushies.to }}</span> of
                <span class="text-rosegold-500">{{ plushies.total }}</span>
                plushies
            </div>
            <nav
                v-if="plushies.total > plushies.limit"
                aria-label="Pagination"
                class="relative z-0 inline-flex justify-center -space-x-px rounded-md shadow-sm"
            >
                <a
                    v-for="(link, i) of plushies.links"
                    :key="i"
                    :class="[
                        link.active
                        ...
                        i === 0 ? 'rounded-l-md' : '',
                        i === plushies.links.length - 1 ? 'rounded-r-md' : '',
                        !link.url
